...
impl Parser {
    ...
    fn is_typename(&self, t: &Token) -> bool {
        match t.ty {
            TokenType::Ident(ref s) => self.find_typedef(s).is_some(),
            TokenType::Int | TokenType::Char | TokenType::Void | TokenType::Struct => true,
            _ => false,
        }
    }
        ...
        match t.ty {
            ...
            TokenType::Struct => {
                ...
                if let Some(ref tag) = tag_opt {
                    if members.is_empty() {
                        ty_opt = self.find_tag(&tag);
                    }
                }
                let mut ty = ty_opt.unwrap_or(Type::new(Ctype::Struct(vec![]), 10));
    ...
    fn ident(&mut self) -> String {
        ...
        if let TokenType::Ident(ref s) = t.ty {
            ...
            t.bad_token("identifier expected")
        }
    }
        ...
        match t.ty {
            ...
            TokenType::Ident(ref name) => {
                if !self.consume(TokenType::LeftParen) {
                    Node::new(NodeType::Ident(name.clone()))
                } else {
                    let mut args = vec![];
            ...
            _ => t.bad_token("primary expected"),
        }
    }
    ...
    fn bit_xor(&mut self) -> Node {
        ...
        while self.consume(TokenType::Caret) {
            lhs = Node::new_binop(TokenType::Caret, lhs, self.bit_and());
        }
        lhs
    }
    ...
    fn param_declaration(&mut self) -> Node {
        ...
        if let Ctype::Array(ary_of, _) = node.ty.ty {
