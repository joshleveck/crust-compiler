        ...
        match self.ty {
            MacroType::Funclike(ref params) => {
                ...
                self.tokens = self
                    ...
                    .scan(false, |is_prev_hashmark, mut t| {
                        if *is_prev_hashmark {
                            ...
                            *is_prev_hashmark = t.ty == TokenType::HashTag;
                        }
                        Some(t)
                    })
                    .collect::<Vec<_>>();
                ...
                self.tokens = self
                    ...
                    .into_iter()
                    .filter_map(|t| {
                        if is_prev_stringize && t.ty == TokenType::HashTag {
...
impl Preprocessor {
    ...
    fn stringize(tokens: &[Token], filename: String, buf: String) -> Token {
        ...
        let len = sb.len();
        Token::new(TokenType::Str(sb, len), 0, 0, 0, buf, filename)
    }
    ...
    fn include(&mut self) {
        ...
        let mut v = tokenize(path, data, self);
